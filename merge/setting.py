...
class UniqueEnv(AbstractEnv):
    ...
    def _reward(self, action: Action) -> float:
        ...
        reward = sum(
            self.config.get(name, 0) * reward for name, reward in rewards.items()  # 重み付きの合計報酬を算出
        )
    ...
    def _rewards(self, action: Action) -> dict[str, float]:
        ...
        return {
            "collision_reward": float(self.vehicle.crashed),  # True or Falseで決定 #1.1の方法の場合
            #"collision_reward": collision, #1.2の方法の場合
            #"collision_reward": ttc_reward, #1.3の方法の場合
            #"collision_reward": collision + ttc_reward, #1.2と1.3の手法の足し算
            #"spacing_reward": spacing_reward, # 追加
            #"high_speed_reward": np.clip(scaled_speed, 0, 1),   # 速度に比例して報酬がもらえる
            #"high_speed_reward": rv,
            #"acceleration_reward": acceleration_reward,  # 追加
            #"lane_change_reward": lane_change_reward,  # 追加
            #"confortable_reward": acceleration_reward + lane_change_reward,
            #"safety_speed_reward": safety_speed_reward,  # 追加
            #"right_lane_reward": lane / max(len(neighbours) - 1, 1),
            #"on_road_reward": float(self.vehicle.on_road),      # 道路上にいれば報酬がもらえる
        }
